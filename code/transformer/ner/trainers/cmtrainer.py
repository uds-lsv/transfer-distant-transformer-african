...
from utils_ner import create_label_map, read_sentences_from_file, batch_to_inputs, load_dataset
...
class ConfusionMatrixTrainer(Trainer):
    ...
    def train(self, exp_config):
        ...
        clean_sentences = read_sentences_from_file(exp_config["data_dir"], "train_clean.tsv")
        clean_noisy_labels_sentences = read_sentences_from_file(exp_config["data_dir"], "train_clean_noisy_labels.tsv")
        ...
        if not "use_true_cm" in exp_config or not exp_config["use_true_cm"]:
            if clean_indices is None:
                ...
                clean_noisy_labels_sentences = np.array(clean_noisy_labels_sentences)[clean_indices]
        else:
        ...
        if not "use_io_estimation" in exp_config or not exp_config["use_io_estimation"]:
            ...
            noise_matrix = ConfusionMatrixTrainer.compute_noise_matrix(clean_sentences, clean_noisy_labels_sentences,
                                                                   exp_config["labels"])
        else:
            logging.info("Using IO format for noise matrix estimation.")
            noise_matrix = ConfusionMatrixTrainer.compute_noise_matrix_io(clean_sentences, clean_noisy_labels_sentences,
                                                                   exp_config["labels"])
    ...
    @staticmethod
    def compute_noise_matrix(sentences_clean, sentences_noisy, labels):
        assert len(sentences_clean) == len(sentences_clean)
        labels_clean = list(itertools.chain(*[sentence.labels for sentence in sentences_clean]))
        labels_noisy = list(itertools.chain(*[sentence.labels for sentence in sentences_noisy]))
    ...
    @staticmethod
    def compute_noise_matrix_io(sentences_clean, sentences_noisy, labels):
    ...
    def train_one_epoch(self, dataloader, optimizer, scheduler, exp_config, type):
        ...
        for step, batch in enumerate(batch_iterator):
            ...
            if type == "clean":
                ...
                loss = compute_loss(logits, inputs["attention_mask"], labels,
                                    len(exp_config["labels"]), is_log_prob=False)
            elif type == "noisy":
                log_probs = model(inputs)
                loss = compute_loss(log_probs, inputs["attention_mask"], labels,
                                    len(exp_config["labels"]), is_log_prob=True)
