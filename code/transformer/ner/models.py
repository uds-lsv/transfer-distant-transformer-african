...
def create_base_model(exp_config):
    ...
    model = AutoModelForTokenClassification.from_pretrained(exp_config["model_name"], config=config)
    model.to(exp_config["device"])
    return model


def compute_loss(logits, input_attention_mask, labels, num_labels, is_log_prob=False):
    """
    ...
    assert loss_fct.ignore_index == -100, "Implementation assumes that ignore index of the loss is -100 and not" \
                                          f"{loss_fct.ignore_index}."

    # We are anyways already setting the label of all the padding tokens to -100,
    # but should not hurt much to ensure that they are really ignored, again.
    active_loss = input_attention_mask.view(-1) == 1
...
def create_optimizer(models, exp_config):
    ...
    # Adapted from Huggingface's NER example
